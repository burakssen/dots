...
Image GenProceduralMap(int width, int height)
{
    ...
    Vector2 octaveOffsets[7];

    std::vector<float> vals = PerlinNoise(width, height, 0, 20, 7, 20, 3, {0, 0}, octaveOffsets, NormalizeMode::Global);

    for (int i = 0; i < width; i++)
    {
        for (int j = 0; j < height; j++)
        {
            pixels[i * height + j] = (Color){(float)abs(i * 255 / width - j * 255 / height) / 2, i * 255 / width, j * 255 / height, 255};
        }
    }
